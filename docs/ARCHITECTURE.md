...
`scripts/main.gd` is the Godot adapter. It owns presentation-scoped pause state, reads touch/keyboard input, advances gravity with frame time only while unpaused, and procedurally draws the board, ghost landing projection, next-piece preview, pause control, and overlays. It delegates every game-state decision to `CletrisRules`.
